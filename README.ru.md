...
1. Требования: **Windows 10/11 с winget** (встроен) и интернет. Всё
   остальное — git, Python 3.11+, Node.js, Google Chrome и OpenCode
   Desktop — скрипт установит сам (могут появиться запросы UAC).
   На Linux/macOS установите вручную (setup.ps1 только для Windows).
2. `git clone https://github.com/PlanbAI/AI_intern.git <папка проекта>`
3. `cd <папка проекта>` и запустите скрипт настройки:
   ```powershell
   powershell -ExecutionPolicy Bypass -File setup.ps1
   ```
   Скрипт сам ставит недостающие зависимости через winget (фолбэк —
   официальные установщики), ставит Python-зависимости (pyyaml,
   jsonschema), устанавливает `@playwright/mcp` (или npx-фолбэк),
   прописывает MCP-сервер в **глобальный** конфиг opencode
   (`~/.config/opencode/opencode.jsonc`) с путями этой машины и
   постоянным браузерным профилем `.opencode/browser-profile`,
   дублирует агента в легаси-путь `.opencode/agent/` (Desktop 1.18.x
   читает его, а не `.opencode/agents/`), создаёт ярлык
   «OpenCode - <проект>» и пересобирает индекс. Идемпотентен —
   повторный запуск безопасен. Флаг `-SkipInstall` — только проверка
   и настройка без установки.
4. Запустите opencode **именно в этой папке** (через созданный ярлык
   или передав путь в OpenCode.exe) — иначе проектный конфиг, плагин
   guard.ts, агент и скилл не загрузятся.
5. Выберите агента **intern** (Tab или в списке агентов).
6. Проверка установки: `python scripts/index.py` → «индекс пересобран:
   процедур 3», и `python scripts/stats.py` (сводка + метрики процедур).
7. Обновление шаблона: `git pull` (конфликты возможны, если меняли файлы
   шаблона; свои процедуры в `agent-memory/procedures/` конфликтов не создают).
...
6. Процедуры запускаются **по смыслу, а не по id**: спроси интерна
   «нет ли встреч в ближайшее время» — он найдёт процедуру календаря через
   `retrieve.py --query "<твоя фраза>"` (или сам выполни ретривал, чтобы
   посмотреть кандидатов). Если подходит несколько процедур — интерн
   показывает нумерованный список, отсортированный по уверенности, и ждёт
   выбора: по умолчанию вариант 1 (самый вероятный), пустой ответ = «1».
7. Загружай скилл intern-agent при работе с памятью.
